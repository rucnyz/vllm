...
def load_schedule_data(results_dir: Path, args) -> tuple[dict, dict, dict, dict]:
    """加载 schedule stats 数据，返回 (fixed_k_results, baseline, dynamic_results, kratio_results)"""
    k_star_results = {}
    ...
    return k_star_results, baseline_metrics, dynamic_results, kratio_results


def load_bench_data(results_dir: Path, args) -> tuple[dict, dict, dict]:
    """加载 vllm bench serve 输出数据，返回 (fixed_k_results, baseline, kratio_results)"""
    k_star_results = {}
    ...
    return k_star_results, baseline_result, kratio_results
...
def plot_all_mode(schedule_data: tuple, bench_data: tuple, output_path: str, no_show: bool):
    ...
    k_star_results_sched, baseline_metrics, dynamic_results, kratio_results_sched = schedule_data
    k_star_results_bench, baseline_result, kratio_results_bench = bench_data

    # 检查数据是否足够 (包括 kratio 结果)
    has_schedule = bool(k_star_results_sched) or bool(kratio_results_sched)
    has_bench = bool(k_star_results_bench) or bool(kratio_results_bench)
...
def main():
    ...
    if args.mode == "all":
        ...
        if not schedule_data[0] and not schedule_data[3] and not bench_data[0] and not bench_data[2]:
    ...
    if mode == "schedule":
        ...
        k_star_results, baseline_result, kratio_results = load_bench_data(results_dir, args)
        if not k_star_results and not kratio_results:
            print("No bench_fixed*.json or bench_kratio_*.json files found!")
            return
        plot_bench_mode(k_star_results, baseline_result, output_path, args.no_show, kratio_results)
