...
# 512x512 场景实验脚本 (从 run_grid_search.sh 提取)
# 只运行 input_len=512, output_len=512 的场景
...
# 输出目录
OUTPUT_DIR="${SCRIPT_DIR}/grid_search_512x512_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTPUT_DIR"
...
echo "  = ${#TB_VALUES[@]} TB × ${#BS_VALUES[@]} BS × 2 Schedulers"
echo ""

# 保存配置
cat > "${OUTPUT_DIR}/experiment_config.json" << EOF
{
    "experiment_type": "grid_search_512x512",
    "model": "${MODEL}",
    "input_len": ${INPUT_LEN},
