...
class ThroughputTestResult:
    ...
    truncated_samples: list[dict] = field(default_factory=list)
...
class PDSchedulerConfig:
    """
    Helper class to manage P/D scheduler environment variables.

    This provides a clean interface for configuring the scheduler and ensures
    proper state management across multiple test runs.
    """

    # Environment variable names
    ENV_USE_PD_SCHEDULER = "VLLM_USE_PD_SCHEDULER"
    ENV_ENABLE_DYNAMIC_KSTAR = "VLLM_PD_ENABLE_DYNAMIC_KSTAR"
    ENV_K_STAR_DYNAMIC = "VLLM_PD_K_STAR_DYNAMIC"
    ENV_P_DYNAMIC = "VLLM_PD_P_DYNAMIC"
    ENV_ALPHA_P = "VLLM_PD_ALPHA_P_DYNAMIC"
    ENV_ALPHA_D = "VLLM_PD_ALPHA_D_DYNAMIC"
    ENV_BETA_D = "VLLM_PD_BETA_D_DYNAMIC"
    ENV_EMA_ALPHA = "VLLM_PD_EMA_ALPHA"
    ENV_UPDATE_INTERVAL = "VLLM_PD_UPDATE_INTERVAL"

    def __init__(self):
        self._saved_state: dict[str, str | None] = {}

    def save_state(self) -> None:
        """Save current environment variable state."""
        env_vars = [
            self.ENV_USE_PD_SCHEDULER,
            self.ENV_ENABLE_DYNAMIC_KSTAR,
            self.ENV_K_STAR_DYNAMIC,
            self.ENV_P_DYNAMIC,
            self.ENV_ALPHA_P,
            self.ENV_ALPHA_D,
            self.ENV_BETA_D,
        ...
        for var in env_vars:
            self._saved_state[var] = os.environ.get(var)

    def restore_state(self) -> None:
        """Restore previously saved environment variable state."""
        for var, value in self._saved_state.items():
            if value is None:
    ...
    def enable_pd_scheduler(self, enable: bool = True) -> None:
        """Enable or disable the P/D competition scheduler."""
        os.environ[self.ENV_USE_PD_SCHEDULER] = "1" if enable else "0"

    def set_dynamic_kstar(self, enable: bool) -> None:
        """Enable or disable dynamic k* updates during inference."""
        os.environ[self.ENV_ENABLE_DYNAMIC_KSTAR] = "1" if enable else "0"

    def set_kstar(self, k_star: int, p: float | None = None) -> None:
        """Set k* (and optionally p) via environment variables."""
        os.environ[self.ENV_K_STAR_DYNAMIC] = str(k_star)
        if p is not None:
            os.environ[self.ENV_P_DYNAMIC] = str(p)

    def set_profiling_params(self, alpha_p: float, alpha_d: float,
                             beta_d: float) -> None:
        """Set profiled timing parameters."""
        os.environ[self.ENV_ALPHA_P] = str(alpha_p)
        os.environ[self.ENV_ALPHA_D] = str(alpha_d)
        os.environ[self.ENV_BETA_D] = str(beta_d)

    def set_online_learning_params(self, ema_alpha: float,
                                   update_interval: int) -> None:
        """Set online learning parameters."""
        os.environ[self.ENV_EMA_ALPHA] = str(ema_alpha)
        os.environ[self.ENV_UPDATE_INTERVAL] = str(update_interval)

    def configure_for_fixed_k_test(self, k: int) -> None:
        """Configure scheduler for fixed-k throughput test."""
        self.set_dynamic_kstar(False)
        self.set_kstar(k)

    def configure_for_dynamic_kstar_test(self, initial_k: int,
                                         initial_p: float) -> None:
        """Configure scheduler for dynamic k* test with online learning."""
        self.set_dynamic_kstar(True)
        self.set_kstar(initial_k, initial_p)
...
def get_scheduler_from_llm(llm):
    """
    Safely get the scheduler from an LLM instance.

    Returns the scheduler object or None if not accessible (e.g., in multiprocess mode).
    """
    try:
        # Path for non-multiprocess mode in vLLM v1
...
def run_throughput_test(llm: LLM, prompts: list[str],
                        ...
                        scheduler_config: PDSchedulerConfig | None = None
    ...
    Args:
        ...
        scheduler_config: Optional PDSchedulerConfig instance for managing
            environment variables. If None, a temporary one is created.

    Returns:
        ThroughputTestResult containing throughput, output lengths, and other stats.
    """
    # Use provided config or create a temporary one
    config = scheduler_config or PDSchedulerConfig()

    # Configure for fixed-k test
    config.configure_for_fixed_k_test(k)
...
def update_kstar_via_env(k_star: int, p: float = None) -> None:
    ...
    Note: Consider using PDSchedulerConfig.set_kstar() for new code.
    """
    os.environ[PDSchedulerConfig.ENV_K_STAR_DYNAMIC] = str(k_star)
    if p is not None:
        os.environ[PDSchedulerConfig.ENV_P_DYNAMIC] = str(p)
...
def run_throughput_test_with_dynamic_kstar(
    ...
    # Configure for dynamic k* test
    config.configure_for_dynamic_kstar_test(initial_k_star, initial_p)
...
def run_experiment_with_real_model(
    ...
    # Set profiled parameters via scheduler config helper
    # The scheduler will read these on the next schedule() call
    scheduler_config.set_profiling_params(
        alpha_p=profile_result.alpha_p,
        alpha_d=profile_result.alpha_d,
        beta_d=profile_result.beta_d
    )
    scheduler_config.set_online_learning_params(
        ema_alpha=ema_alpha,
        update_interval=kstar_update_interval
    )
    print("\nProfiling parameters set for scheduler:")
    print(f"  α_p={profile_result.alpha_p:.6f}, α_d={profile_result.alpha_d:.6f}, "
          f"β_d={profile_result.beta_d:.6f}")
    ...
    os.environ[PDSchedulerConfig.ENV_P_DYNAMIC] = str(offline_p_estimated)
    ...
    # Create scheduler config for all tests
    scheduler_config = PDSchedulerConfig()
    ...
    for i, k in enumerate(k_values):
        ...
        throughput_by_k[k] = result.throughput

        # Update overall progress
        overall_pbar.update(requests_per_k)

        # Show result for this k
        tqdm.write(f"  k={k}: throughput={result.throughput:.2f} req/s")
    ...
    if k_star not in throughput_by_k:
        ...
        throughput_kstar = result.throughput
    ...
    # Try to enable timeline recording using helper function
    timeline_data = None
    scheduler = get_scheduler_from_llm(llm)
    if scheduler is not None:
        try:
            scheduler.enable_pd_timeline(True)
            print("Timeline recording enabled")
        except AttributeError:
            print("Note: Scheduler does not support timeline recording")
    else:
        print("Note: Could not access scheduler (multi-process mode)")

    final_result = run_throughput_test(
    ...
    collected_output_lengths = final_result.output_lengths
    truncated_samples = final_result.truncated_samples

    # Retrieve timeline data using helper function
    scheduler = get_scheduler_from_llm(llm)
    if scheduler is not None:
        try:
            timeline_data = scheduler.get_pd_timeline()
            scheduler.enable_pd_timeline(False)
            print(f"Collected timeline data for "
                  f"{len(timeline_data.get('requests', {}))} requests")
        except AttributeError:
            pass
