...
|----------|---------|-------------|
| `--model` | `Qwen/Qwen3-8B` | HuggingFace model name or local path |
| `--tensor-parallel-size` | `1` | Number of GPUs for tensor parallelism |
| `--disable-thinking` | `false` | Disable thinking mode for Qwen3 models (default: enabled) |
...
| Argument | Default | Description |
|----------|---------|-------------|
| `--scheduler` | `pd` | Scheduler type: `pd` (P/D competition) or `default` (original vLLM) |
| `--disable-multiprocessing` | `false` | Disable vLLM multiprocessing (required for timeline recording with TP) |
...
| `--max-input-tokens` | `10240` | Skip samples exceeding this input length |
...
| Argument | Default | Description |
|----------|---------|-------------|
| `--num-requests` | `1024` | Number of requests for throughput testing |
| `--batch-size` | `64` | Scheduler batch size N |
| `--max-output-tokens` | `4096` | Maximum output tokens per request |
| `--custom-prompts` | `None` | Path to JSON file with filtered prompts (overrides `--dataset`) |
| `--exclude-degenerate-throughput` | `false` | Exclude degenerate outputs (repetition loops) from throughput calculation |
...
| Argument | Default | Description |
|----------|---------|-------------|
| `--offline-p-multiplier` | `1.0` | Offline samples = multiplier × kstar-update-interval |
| `--ema-alpha` | `0.2` | EMA smoothing factor for online p updates (higher = more weight on recent observations) |
...
| Argument | Default | Description |
|----------|---------|-------------|
| `--num-prefill-samples` | `64` | Number of samples for prefill time measurement |
| `--num-decode-repeats` | `5` | Repetitions per batch size for decode profiling |
...
### 1b. Using Original vLLM Scheduler (Baseline Comparison)
...
### 10b. Qwen3 Without Thinking Mode

Disable thinking mode for Qwen3 models (useful for shorter outputs):
...
### 12. Using Custom Filtered Prompts

Use pre-filtered prompts from a JSON file (e.g., from `filter_samples_by_thinking.py`):

```bash
python experiments/kstar_vs_khat_real_model.py \
    --model Qwen/Qwen3-8B \
    --custom-prompts ./filtered_prompts.json \
    --num-requests 500
```
...
9. **Scheduler Selection**: Use `--scheduler default` to run with the original vLLM scheduler for baseline comparison.
10. **Qwen3 Thinking Mode**: Qwen3 models support thinking mode (enabled by default). Use `--disable-thinking` to disable it for shorter outputs.
...
The experiment script includes several helper classes for cleaner code organization:

#### `PDSchedulerConfig`
Centralized management of P/D scheduler environment variables:
```python
config = PDSchedulerConfig()
config.enable_pd_scheduler(True)
config.set_profiling_params(alpha_p, alpha_d, beta_d)
config.set_online_learning_params(ema_alpha, update_interval)
config.configure_for_fixed_k_test(k=5)
config.configure_for_dynamic_kstar_test(initial_k=3, initial_p=0.01)
```

Can also be used as a context manager to save/restore state:
```python
with PDSchedulerConfig() as config:
    config.configure_for_fixed_k_test(k=5)
    # ... run test ...
# Environment variables restored automatically
```

#### `ThroughputTestResult`
Structured return type for throughput tests:
```python
@dataclass
class ThroughputTestResult:
    throughput: float          # requests/second
    output_lengths: list[int]  # Output token lengths
    truncated_samples: list    # Samples that hit max_tokens
    degenerate_count: int      # Number of repetition-loop outputs
    total_completed: int       # Total completed requests
    elapsed_time: float        # Total elapsed time
```

#### `get_scheduler_from_llm(llm)`
Safely retrieves the scheduler from an LLM instance, returning `None` in multiprocess mode instead of raising exceptions.

### Key Functions

| Function | Description |
|----------|-------------|
| `run_throughput_test()` | Run throughput test with fixed k |
| `run_throughput_test_with_dynamic_kstar()` | Run with dynamic k* online learning |
| `profile_model()` | Profile α_p, α_d, β_p, β_d timing parameters |
| `compute_analytical_kstar()` | Compute k* using Proposition 1 formula |
| `compute_optimal_batch_size_N()` | Compute N* under memory constraints |
