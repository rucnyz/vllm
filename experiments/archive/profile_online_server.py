...
Usage:
    ...
    # Option 1: Use synthetic workload (RECOMMENDED for accurate α/β)
...
def create_synthetic_workload(tokenizer, num_prompts: int = 200,
                              min_length: int = 50,
                              max_length: int = 8000) -> list[str]:
    """
    Create a fully synthetic workload with controlled input lengths.

    This is RECOMMENDED for accurate α_p and β_p estimation because:
    1. Precise control over input lengths for better linear regression
    2. No external dataset dependency
    3. Reproducible results
    4. Even distribution across length range
    ...
    Returns:
        List of prompts with evenly distributed lengths
    """
    # Base text for generating prompts (diverse vocabulary for realistic tokenization)
    base_text = """
    The quick brown fox jumps over the lazy dog. This is a sample text that will
    be repeated to create prompts of various lengths for profiling purposes.
    Machine learning models require extensive testing to ensure reliable performance
    across different input sizes. Natural language processing has revolutionized
    how we interact with computers and analyze text data. Deep learning architectures
    such as transformers have enabled breakthrough capabilities in understanding
    and generating human language. The attention mechanism allows models to focus
    on relevant parts of the input when producing outputs.
    """

    base_tokens = tokenizer.encode(base_text.strip())
    if len(base_tokens) < 20:
        # Fallback if tokenization is too aggressive
        base_tokens = list(range(1000, 1100))  # Use token IDs directly

    prompts = []

    # Generate evenly distributed target lengths (log scale for better coverage)
    # Log scale ensures good coverage of both short and long sequences
    log_lengths = np.logspace(
        np.log10(min_length),
        np.log10(max_length),
        num_prompts
    )
    target_lengths = sorted(set(int(length) for length in log_lengths))

    print(f"Generating {len(target_lengths)} synthetic prompts "
          f"({min_length}-{max_length} tokens)...")
    ...
    # Shuffle to avoid sequential length patterns during profiling
...
def main():
    ...
    parser.add_argument('--synthetic', action='store_true',
                        help='Use synthetic workload (RECOMMENDED for accurate α/β)')
    parser.add_argument('--dataset', type=str, default='alpaca',
                        choices=['alpaca', 'sharegpt', 'lmsys'],
                        help='Dataset to use (ignored if --synthetic is set)')
    ...
    parser.add_argument('--max-input-tokens', type=int, default=8000,
                        help='Max input token length for synthetic/dataset prompts')
    parser.add_argument('--min-input-tokens', type=int, default=50,
                        help='Min input token length (only for --synthetic mode)')
    ...
    # Generate workload
    if args.synthetic:
        # Use synthetic workload for accurate α/β measurement
        print("\nUsing SYNTHETIC workload (recommended for accurate α/β)")
        num_prompts = (args.num_prefill_samples + args.num_p_samples
                       + args.batch_size * 2 + 100)
        prompts = create_synthetic_workload(
            tokenizer,
            num_prompts=num_prompts,
            min_length=args.min_input_tokens,
    ...
    json_path = os.path.join(
        args.output_dir, f"{workload_name}_online_profiling_results.json")
    with open(json_path, 'w') as f:
        json.dump(output_data, f, indent=2)
    print(f"\nSaved: {json_path}")

    # Plot results
    plot_profiling_results(result, args.output_dir, f"{workload_name}_online")
