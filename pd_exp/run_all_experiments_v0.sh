...
if [[ "$EXPERIMENTS" == *"sharegpt"* ]]; then
    if [ -f "$SHAREGPT_PROMPTS" ]; then
        run_experiment "ShareGPT" "$SHAREGPT_PROMPTS" 500 false "${SCRIPT_DIR}/real/run_grid_search_v0.sh"
    else
...
if [[ "$EXPERIMENTS" == *"numina_math"* ]]; then
    if [ -f "$NUMINA_PROMPTS" ]; then
        run_experiment "numina_math" "$NUMINA_PROMPTS" 4000 true "${SCRIPT_DIR}/real/run_grid_search_v0.sh"
    else
...
if [[ "$EXPERIMENTS" == *"longbench"* ]]; then
    if [ -f "$LONGBENCH_PROMPTS" ]; then
        run_experiment "longbench" "$LONGBENCH_PROMPTS" 20 false "${SCRIPT_DIR}/real/run_grid_search_v0.sh"
    else
